#!/usr/bin/env bash
# jmux bash integration — CWD reporting, git branch, PR polling, port kicks,
...
#
# Auto-injected via BASH_ENV when JMUX_SOCKET is set (injected by jmux into
...
[[ -n "$_JMUX_BASH_INTEGRATION_LOADED" ]] && return 0
_JMUX_BASH_INTEGRATION_LOADED=1
...
_jmux_send() {
  ...
  if command -v ncat >/dev/null 2>&1; then
    echo "$msg" | ncat -U "$JMUX_SOCKET" 2>/dev/null
  elif command -v socat >/dev/null 2>&1; then
    echo "$msg" | socat - UNIX-CONNECT:"$JMUX_SOCKET" 2>/dev/null
  elif command -v nc >/dev/null 2>&1; then
    echo "$msg" | nc -U "$JMUX_SOCKET" -w 1 2>/dev/null
  fi
}

_jmux_send_bg() {
  _jmux_send "$1" >/dev/null 2>&1 &
  disown 2>/dev/null
}
...
# ── CWD reporting ────────────────────────────────────────────────────
_jmux_report_pwd() {
  _jmux_send_bg "report_pwd \"$PWD\" $(_jmux_flags)"
}
...
_jmux_update_git_branch_sync() {
  ...
  if [[ -n "$branch" ]]; then
    ...
    _jmux_send "report_git_branch ${branch}${dirty} $(_jmux_flags)" \
      >/dev/null 2>&1
...
_jmux_report_tty() {
  ...
  [[ -n "$tty_name" ]] && _jmux_send_bg "report_tty $tty_name $(_jmux_flags)"
}
...
if command -v claude >/dev/null 2>&1; then
  claude() {
    ...
    if [[ "$inject" == 1 ]]; then
      ...
      if [[ -n "$_jmux_sid" ]]; then
        _jmux_send_bg "report_agent_session claude $_jmux_sid $(_jmux_flags)"
...
# ── Port scanning kick ──────────────────────────────────────────────
_jmux_ports_kick() {
  _jmux_send_bg "ports_kick"
}

# ── Shell state reporting ────────────────────────────────────────────
_jmux_report_prompt() {
  _jmux_send_bg "report_shell_state prompt $(_jmux_flags)"
}

_jmux_report_running() {
  _jmux_send_bg "report_shell_state running $(_jmux_flags)"
}
...
_jmux_kill_process_tree() {
  local pid="$1"
  local -a descendants
  mapfile -t descendants < <(_jmux_child_pids "$pid")
...
trap '_jmux_cleanup' EXIT

# ── Hook into bash prompt lifecycle ──────────────────────────────────
_jmux_last_exit=0
_jmux_preexec_fired=0

_jmux_prompt_command() {
  local exit_code=$?
...
if [[ -z "$PROMPT_COMMAND" ]]; then
  PROMPT_COMMAND="_jmux_prompt_command"
elif [[ "$PROMPT_COMMAND" != *"_jmux_prompt_command"* ]]; then
  PROMPT_COMMAND="_jmux_prompt_command;$PROMPT_COMMAND"
fi
...
if [[ "${BASH_VERSINFO[0]}" -ge 5 ]] || \
  ...
  _jmux_ps0_preexec() {
    if [[ "$_jmux_preexec_fired" -eq 0 ]]; then
      _jmux_preexec_fired=1
      _jmux_osc133_command_start
      _jmux_report_running
      _jmux_start_git_watcher
    fi
  }
  PS0='$(_jmux_ps0_preexec)'
else
  # Fallback: DEBUG trap for older Bash
  _jmux_debug_trap() {
    case "$BASH_COMMAND" in
      _jmux_prompt_command|_jmux_report_pwd|_jmux_update_git_branch|\
      _jmux_report_prompt|_jmux_stop_git_watcher|_jmux_osc133_*|\
      _jmux_ps0_preexec|_jmux_cleanup)
        return ;;
    esac
