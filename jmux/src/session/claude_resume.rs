...
//! `claude` process by the `JMUX_PANEL_ID` env var jmux injects into the shell
...
/// Read the `JMUX_PANEL_ID` value out of a NUL-delimited `environ` blob.
fn panel_id_from_environ(environ: &[u8]) -> Option<Uuid> {
    for kv in environ.split(|&b| b == 0) {
        if let Some(v) = kv.strip_prefix(b"JMUX_PANEL_ID=") {
...
/// single `/proc` walk. Keyed by `JMUX_PANEL_ID`; the value is the cwd of the
/// panel's shell — the process that is a direct child of this jmux process, so
...
pub fn all_local_panel_cwds() -> HashMap<Uuid, String> {
    // Only consider processes that are DIRECT children of this jmux process. A
    // panel's shell (or a directly-launched agent) is always such a child, so
    // this both scopes the walk to *this* jmux instance — a second jmux running
...
mod tests {
    ...
    fn test_encode_project_dir() {
        assert_eq!(encode_project_dir("/home/u/src/jmux"), "-home-u-src-jmux");
    ...
    fn test_panel_id_from_environ() {
        let id = Uuid::new_v4();
        let blob = format!("PATH=/bin\0JMUX_PANEL_ID={id}\0HOME=/root\0");
        assert_eq!(panel_id_from_environ(blob.as_bytes()), Some(id));
        assert_eq!(panel_id_from_environ(b"PATH=/bin\0HOME=/root\0"), None);
        assert_eq!(panel_id_from_environ(b"JMUX_PANEL_ID=not-a-uuid\0"), None);
    }
