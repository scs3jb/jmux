...
//!   - `<workspace>/.jmux/dock.json`
//!   - `~/.config/jmux/dock.json`
//!
...
/// Path to the global dock config (`~/.config/jmux/dock.json`).
...
pub fn load(workspace_dir: &str) -> Vec<DockControl> {
    ...
    if !workspace_dir.is_empty() {
        paths.push(Path::new(workspace_dir).join(".jmux/dock.json"));
    }
...
fn build_into(dock: &gtk4::Box, workspace_dir: &str, state: &Rc<AppState>) {
    ...
    if controls.is_empty() {
        let hint = gtk4::Label::new(Some(
            "No dock controls configured.\n\nAdd a dock.json with:\n\n{\n  \"controls\": [\n    { \"id\": \"git\",\n      \"title\": \"Git\",\n      \"command\": \"lazygit\" }\n  ]\n}\n\nin .jmux/dock.json (this project)\nor ~/.config/jmux/dock.json (global).",
        ));
