...
/// `system.task_manager` (`jmux top`) — open the Task Manager window.
...
/// `system.dock` (`jmux dock`) — show the Dock panel.
...
/// `system.run_command` (`jmux run <name>`) — run a jmux.json custom command.
