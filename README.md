...
- **Dock** — right-side column of terminal "controls" (lazygit, log tails, watchers) from `.cmux/dock.json` / `~/.config/cmux/dock.json` (`id`/`title`/`command`/`cwd`/`height`); toggle with Ctrl+Shift+L or the palette
