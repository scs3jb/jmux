...
// to the browser instead of the filesystem (jmux/bin/jmux `open` case).
var urlPattern = regexp.MustCompile(`^https?://`)

// runBashCompat implements the human-facing command vocabulary of the local
// bash `jmux` CLI (jmux/bin/jmux) so that commands typed in a remote shell —
// `jmux open`, `jmux notes`, `jmux tree`, `jmux list`, … — behave identically
...
func runBashCompat(socketPath, name string, args []string, jsonOutput bool, refreshAddr func() string) (int, bool) {
	...
	case "send":
		if len(args) == 0 {
			fmt.Fprintln(os.Stderr, "usage: jmux send <text>")
			return 2, true
		}
...
// does (jmux/bin/jmux `read|read-screen` case): --scrollback, --lines=N,
...
func cmdOpen(socketPath string, args []string, jsonOutput bool, refreshAddr func() string) int {
	if len(args) == 0 {
		fmt.Fprintln(os.Stderr, "usage: jmux open <path-or-url>...")
		return 2
	}
	rc := 0
	var files []any
	for _, t := range args {
		...
		default:
			info, err := os.Stat(t)
			if err != nil {
				fmt.Fprintf(os.Stderr, "jmux open: not found: %s\n", t)
				rc = 1
				continue
			}
			if info.IsDir() {
