...
func TestWrapperBinaryDispatchesIntoCLI(t *testing.T) {
	if os.Getenv("JMUXD_REMOTE_MAIN_HELPER") == "1" {
		separator := 0
	...
	wrapperPath := filepath.Join(t.TempDir(), "jmuxd-remote-current")
	...
	cmd.Env = append(os.Environ(), "JMUXD_REMOTE_MAIN_HELPER=1")
...
func TestProxyStreamRoundTrip(t *testing.T) {
	...
	t.Setenv("JMUXD_PROXY_ALLOW_PRIVATE", "1")
...
func TestProxyOpenBlockedLoopback(t *testing.T) {
	t.Setenv("JMUXD_PROXY_ALLOW_PRIVATE", "0")
...
func TestProxyOpenAllowPrivateEnvOverride(t *testing.T) {
	// With JMUXD_PROXY_ALLOW_PRIVATE=1, connections to private IPs skip the
	...
	if resp.Error != nil && resp.Error.Code == "blocked" {
		t.Errorf("JMUXD_PROXY_ALLOW_PRIVATE=1 should bypass the block, got 'blocked'")
	}
}
