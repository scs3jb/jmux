...
// Set JMUXD_PROXY_ALLOW_PRIVATE=1 to permit connections to private/loopback
...
func shouldRunCLIForInvocation(argv0 string, args []string) bool {
	...
	if !strings.HasPrefix(base, "jmuxd-remote") || len(args) == 0 {
		return false
	}
...
func usage(w io.Writer) {
	...
	_, _ = fmt.Fprintln(w, "  jmuxd-remote version")
	_, _ = fmt.Fprintln(w, "  jmuxd-remote serve --stdio")
	_, _ = fmt.Fprintln(w, "  jmuxd-remote cli <command> [args...]")
}
...
func (s *rpcServer) handleProxyOpen(req rpcRequest) rpcResponse {
	...
	if os.Getenv("JMUXD_PROXY_ALLOW_PRIVATE") != "1" {
		...
		for _, addr := range addrs {
			...
			if ip != nil && isBlockedAddr(ip) {
				return rpcResponse{
					...
					Error: &rpcError{
						Code:    "blocked",
						Message: fmt.Sprintf("proxy.open: destination %q resolves to a blocked address (%s); set JMUXD_PROXY_ALLOW_PRIVATE=1 to allow", host, addr),
					},
				}
			}
		}
